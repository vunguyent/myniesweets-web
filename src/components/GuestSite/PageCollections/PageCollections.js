...
import { fetchListActiveCakesByCollectionAPI } from 'actions/ApiCall'
...
function PageCollections() {
  ...
  useEffect(() => {
    ...
    fetchListActiveCakesByCollectionAPI(collectionId, searchPath).then((res) => {
      setCakes(res.cakes)
      setTotalPages(Math.ceil(res.totalCakes / 2))
    })
  }, [type, selected, collectionId])
        ...
        <div className="pt-4 pb-4 sm:pb-8 px-4 md:px-16">
          <div className="flex flex-col sm:flex-row item-center py-4 md:py-6">
            <div className='flex text-base sm:text-lg font-bold text-gray-600 uppercase items-center mr-2 sm:mr-8'>
              {
                collectionId && collectionId==='all' ? <span >TẤT CẢ SẢN PHẨM</span> :
                  !isEmpty(cakeTypes) && cakeTypes.map((cakeType, i) => (
                    (cakeType._id === collectionId) &&
                        <span key={i}>{cakeType.title}</span>
                  ))
              }
            </div>
          ...
          { page < totalPages &&
            <div className='flex items-center justify-center'>
              <button className='bg-mn-yellow text-sm text-mn-blue font-bold hover:bg-messenger-blue hover:text-white px-4 py-2 rounded-full' onClick={() => nextPage(page)}>Xem thêm mẫu</button>
            </div>
          }
