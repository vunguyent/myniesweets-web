...
function CreateCakeTypeModal({ show, onClose, onCreateNewCakeType }) {
  const { register, handleSubmit, reset, formState: { errors } } = useForm()
  const cakeTypeTitleInputRef = useRef(null)

  const onSubmit = (data) => {
    onCreateNewCakeType(data).then( () => {
      onClose()
      // resetForm()
      reset()
    })
  ...
  return (
    <Transition.Root show={show} as={Fragment}>
      <Dialog as="div" className="relative z-10" initialFocus={cakeTypeTitleInputRef} onClose={onClose}>
        <Transition.Child
          as={Fragment}
...
export default CreateCakeTypeModal